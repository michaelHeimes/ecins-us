...
$news_countries = get_terms( 'news-country', array(
	'hide_empty' => true,
) );
...
foreach ( $news_countries as $country ) {
	$get_countries[] = $country->term_id;
}
...
if ( $get_news->have_posts() ) {
	while ( $get_news->have_posts() ) {
		...
		foreach ( $news_countries as $country ) {
			if ( has_term( $country->term_id, 'news-country', get_the_ID() ) ) {
				if ( ! array_key_exists( $country->name, $sorted_news ) ) {
					$sorted_news[ $country->name ] = array();
				}

				$sorted_news[ $country->name ][] = $post;
			}
		}
		...
		<div class="content-grid--filters">
			<div class="filter-box news-type" data-filter="news-type">
				...
				<div class="filter-values">
					...
					<?php } ?>
					<?php foreach ( $news_countries as $country ) { ?>
						<div class="filter-checkbox">
							<input type="checkbox" value="<?php echo $country->term_id; ?>" id="filter-news-type-<?php echo $country->slug; ?>">
							<label for="filter-news-type-<?php echo $country->slug; ?>">
